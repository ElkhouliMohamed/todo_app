            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
                ...
                <div className="flex items-end justify-between">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 mt-1">{formattedDate} • You have {pendingCount} pending tasks</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-12 gap-6">
                    ...
                    <div className="col-span-12 lg:col-span-4 space-y-6">
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl shadow-sm">
                            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-6">Productivity Score</h3>
                            <div className="flex flex-col items-center">
                                <div className="relative w-40 h-40">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        <span className="text-3xl font-black">{productivityScore}%</span>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-xl">
                                ...
                                <p className="text-2xl font-black">{stats?.completed || 0}</p>
                    ...
                    <div className="col-span-12 lg:col-span-5 space-y-4">
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl h-full shadow-sm flex flex-col">
                            ...
                            <div className="space-y-3 flex-1">
                                {recentTasks && recentTasks.map(task => (
                                    <div key={task.id} className="group flex items-center gap-4 p-4 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-transparent hover:border-slate-200 dark:hover:border-slate-700 transition-all cursor-pointer">
                                        <div className={`w-6 h-6 rounded border-2 flex items-center justify-center ${task.status === 'completed' ? 'border-primary bg-primary' : 'border-slate-300 dark:border-slate-600 group-hover:border-primary'}`}>
                                            {task.status === 'completed' && <span className="material-symbols-outlined text-white text-base">check</span>}
                                            {task.status !== 'completed' && <span className="material-symbols-outlined text-primary text-base opacity-0 group-hover:opacity-100">check</span>}
                                        </div>
                                        <div className={`flex-1 ${task.status === 'completed' ? 'line-through opacity-50' : ''}`}>
                                            <p className="text-sm font-bold truncate">{task.title}</p>
                                            <p className="text-xs text-slate-500 font-medium">{task.due_date ? format(new Date(task.due_date), 'MMM d, h:mm a') : 'No due date'}</p>
                                        </div>
                            ...
                            <Link href={route('tasks.create')}>
                                <button className="w-full mt-6 py-3 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg text-slate-400 text-sm font-bold hover:border-primary hover:text-primary transition-all">
                                    + Add another task
                                </button>
                            </Link>
                        </div>
                    ...
                    <div className="col-span-12 lg:col-span-3 space-y-4">
                        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-xl h-full shadow-sm">
                            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400 mb-6">Upcoming</h3>
                            <div className="relative pl-6 space-y-8 before:content-[''] before:absolute before:left-0 before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100 dark:before:bg-slate-800">
                                {upcomingTasks && upcomingTasks.map((task, index) => (
                                    <div key={task.id} className="relative">
                                        ...
                                        <p className="text-xs text-slate-500 mt-1">{task.description ? task.description.substring(0, 30) + '...' : 'No description'}</p>
                                    </div>
                                ))}
